...
def config_dataset(config):
    ...
    config["data_path"] = "/datasets/" + config["datasets"] + "/"
    ...
    if config["datasets"] in ["nuswide_21_m", "nuswide_81_m"]:
        config["data_path"] = "data/nuswide_81"
...
def validate(config, Best_mAP, test_loader, dataset_loader, net, bit, epoch, num_image,):
    ...
    logger.info(f"{config['info']} epoch:{epoch + 1} bit:{bit} dataset1s:{config['datasets']} MAP:{mAP} Best MAP: {Best_mAP}")
