...
class MultiScale_feature_fusion_module(nn.Module):
    def __init__(self, out_channels):
        super(MultiScale_feature_fusion_module, self).__init__()

        # To reduce the number of model parameters, we use the following approach, where the results do not change
        self.convL1 = nn.Conv2d(out_channels, out_channels,(1,11),padding=(0,5))
        self.convL2 = nn.Conv2d(out_channels, out_channels, (11, 1), padding=(5, 0))

        self.convM1 = nn.Conv2d(out_channels, out_channels, (1,7), padding=(0,3))
        self.convM2 = nn.Conv2d(out_channels, out_channels, (7,1), padding=(3,0))

        self.convS1 = nn.Conv2d(out_channels, out_channels, (1,3), padding=(0,1))
        self.convS2 = nn.Conv2d(out_channels, out_channels, (3,1), padding=(1,0))
        # self.poolL = nn.MaxPool2d(14,stride=1)
        # self.poolM = nn.MaxPool2d(7,stride=7)
        # self.poolS = nn.MaxPool2d(2,stride=2)
        self.maxpool = nn.MaxPool2d(7, stride=7)
        # self.maxpool = nn.AvgPool2d(7, stride=7)


    def forward(self, feature):

        xL = self.convL1(feature)
        print(f'xL size {xL.shape}')
        xL = self.convL2(xL)
        print(f'xL size {xL.shape}')
        xM = self.convM1(feature)
        print(f'xM size {xM.shape}')
        xM = self.convM2(xM)
        print(f'xM size {xM.shape}')

        xS = self.convS1(feature)
        print(f'xS size {xM.shape}')
        xS = self.convS2(xS)
        print(f'xS size {xM.shape}')
        ...
        xM = xM.reshape(B,C,-1)
        xS = xS.reshape(B,C,-1)
        feature = torch.cat((xL,xM,xS),dim=2)
        # feature = torch.cat((xL, xS), dim=2)
        feature = torch.flatten(feature,1,2)
        return feature
...
class HashLayer(nn.Module):
    def __init__(self, config, hash_bit):
        ...
        #self.FcLayer = nn.Linear(config, hash_bit)
        self.act = nn.Tanh()


    def forward(self, feature):

        #hash_re = self.head(feature)
...
class Nest(nn.Module):
    ...
    def __init__(self, config,
        ...
        self.convL1 = nn.Conv2d(embed_dims[-1], embed_dims[-1], (1, 11), padding=(0, 5))
        self.convL2 = nn.Conv2d(embed_dims[-1], embed_dims[-1], (11, 1), padding=(5, 0))
        # Classifier
        self.global_pool, self.head_cls = create_classifier(config["bit"], self.num_classes, pool_type=global_pool)
        #self.global_pool, self.head_cls = create_classifier(512, self.num_classes, pool_type=global_pool)
    ...
    def forward_features(self, x):
        ...
        #
        #
        # x_2 = x_2.reshape(B, C, -1)
        # feature = torch.cat((x_2, x_2, x_2), dim=2)
        # feature=torch.flatten(feature,1,2)
        return x_2
    ...
    def forward_head(self,x,pre_logits:bool=False):
        ...
        x=self.head_drop(x)
        print('x2', x.shape)
        return x if pre_logits else self.head_cls(x)
    def forward(self, x):
        """ x shape (B, C, H, W)
        """
        x = self.forward_features(x)
        #x = self.forward_fusion(x)
